  ...
  <div class="row">
    ...
    <div v-if="actions==='LIST'" class="row">
      <div v-if="allIds.length>0">
        ...
        <div class="col-md-2">
          <button class="btn btn-info btn-wd" @click="UpdateState">Enviar a pre-aprobacion</button>
        </div>
      </div>
    </div>
    ...
    methods: {
      ...
      generatePDF () {
        ...
        for (let i = 0; i < this.careers.length; i++) {
          ...
          doc.autoTable({
            ...
            head: [['Docente', 'Categ', 'Modal', 'Tarea', 'Alumno', 'Acta', 'Fecha', 'Horas', 'Costo Hora', 'Total Bruto', 'TotalNeto', 'Observaciones']],
            body: tableBody,
            theme: 'grid',
          ...
          let resultBody = []
          resultBody.push([careerResults[0].Total_Bruto, careerResults[0].Total_Neto])
          doc.autoTable({
            startY: doc.previousAutoTable.finalY,
            ...
            theme: 'grid',
            body: resultBody,
            columnStyles: {0: {cellWidth: 15.6}, 1: {cellWidth: 15.4}},
        ...
        doc.autoTable({
          ...
          margin: {left: 203.5},
          columnStyles: {0: {cellWidth: 15.5}, 1: {cellWidth: 15.5}},
          styles: {cellPadding: 0.5, fontSize: 8, fillColor: [222, 222, 222], fontStyle: 'bold'}
        })
        doc.setFontSize(8)
      ...
      loadAllTotals () {
        ...
        axios.get('PDFReportBody?part=FinalResult;' + this.estado + ';' + this.origen)
          .then(response => {
            response.data.forEach(function (element) {
              final.push([element.Total_Bruto, element.Total_Neto])
            })
          })
          .catch(error => console.log(error))
      },
